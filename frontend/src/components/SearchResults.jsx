...
export default function SearchResults({response,onSearch}){
 ...
 return <section className="results"><div className="section-heading"><div><span className="eyebrow">Best match</span><h2>Here’s the service you need</h2></div><p>Matched for “{response.query}”</p></div><div className="results-grid">{response.results.map(s=><ServiceCard key={s.intent||s.serviceName} service={s}/>)}</div></section>
}
