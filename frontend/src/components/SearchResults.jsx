...
export default function SearchResults({response,onSearch}){
 ...
 return <section className="results"><div className="section-heading"><div><span className="eyebrow">{multiple?'Related services':'Best match'}</span><h2>{multiple?'Choose the service you need':'Here’s the service you need'}</h2></div><p>{multiple?`${response.results.length} possible matches`:'Matched'} for “{response.query}”</p></div><div className="results-grid">{response.results.map(s=><ServiceCard key={s.intent||s.serviceName} service={s}/>)}</div></section>
}
